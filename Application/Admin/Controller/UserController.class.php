...
class UserController extends CommonController {
    ...
    public function save(){
    	...
    	if(isset($data['id'])){
    		...
    		if($result){
    			$this->success($data['name'].'更新成功',U('admin/user/lists'),3);
    		}else{
    			$this->error($data['name'].'更新失败',U('admin/user/lists'),3);
    		}
    		...
    		if($result){
    			$this->success($data['name'].'添加成功',U('admin/user/lists'),3);
    		}else{
    			$this->error($data['name'].'添加失败',U('admin/user/lists'),3);
    		}
    	}
    ...
    public function savepwd(){
    	...
    	if($data['pwd']==$data['confirm_pwd']){
            ...
            $this->error('两次密码不一致',U('admin/user/lists'),3);
        }
    }
}