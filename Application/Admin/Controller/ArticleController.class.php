...
class ArticleController extends Controller {
    ...
    public function save(){
    	...
    	if(isset($data['id'])){
    		...
    		if($result){
    			$this->success($data['name'].'更新成功',U('admin/article/lists'),3);
    		}else{
    			$this->error($data['name'].'更新失败',U('admin/article/lists'),3);
    		}
    		...
    		if($result){
    			$this->success($data['name'].'添加成功',U('admin/article/lists'),3);
    		}else{
    			$this->error($data['name'].'添加失败',U('admin/article/lists'),3);
    		}
    	}
