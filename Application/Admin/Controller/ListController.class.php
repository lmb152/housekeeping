...
class ListController extends CommonController {
    ...
    public function lists(){
    	...
    	if($artkind['issingle']){
    		...
    		if($article){
    			$url=U('admin/list/edit',array('akid' => $id, 'arid'=>$article['id']));
    		}else{
    			$url=U('admin/list/edit',array('akid' => $id));
    		}
            ...
            //分页跳转的时候保证查询条件
            $list=$articles->where($condition)->order('released desc')->limit($Page->firstRow.','.$Page->listRows)->select();
            // $p->lastSuffix=false;
    ...
    public function edit(){
    	...
    	$article=M('article');
    	if(!$akid){
    		$this->error('并不知道你要编辑哪个类别下的文章，只好返回首页',U('admin/index'),3);
    	}
    	if($arid){
    		$condition['id']=$arid;
    		...
    		if($article){
    			...
    			$this->error('无此文章',U('admin/list/lists',array('id'=>$akid)),3);
    		}
    	}else{
    		$this->assign('article','');
    ...
    public function save(){
    	...
    	if($result){
    		$this->success('操作成功',U('admin/list/lists',array('id'=>$data['artkind'])),3);
		}else{
			$this->error('操作失败',U('admin/list/lists',array('id'=>$data['artkind'])),3);
		}
    }
}